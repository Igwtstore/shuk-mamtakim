...
grant select (fraccion_de, fraccion_cant, sueltas) on productos to anon, authenticated;
...
begin
  ...
  select id, stock, sueltas, unidades_por_paquete, costo, moneda, dueno, fraccion_de
  ...
  upp := greatest(1, coalesce(m.unidades_por_paquete, 1));
  ...
  new.moneda := m.moneda;
  new.dueno  := m.dueno;
  new.unidades_por_paquete := k;          -- la tienda muestra "trae 3 · $ X c/u" sola
  new.sueltas := 0;
...
begin
  if coalesce(new.fraccion_de, '') = '' and (
    ...
    or new.moneda is distinct from old.moneda or new.dueno is distinct from old.dueno) then
...
begin
  ...
  select id, nombre, stock, sueltas, unidades_por_paquete into m from productos where id = f.fraccion_de for update;
  if not found then
  ...
  return json_build_object('antes', antes, 'despues', despues, 'nombre', f.nombre, 'fraccion', true,
    ...
    'cerradas', m.stock, 'sueltas', m.sueltas);
end $$;
